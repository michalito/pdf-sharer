export function substituteInOrder(
  source: number[],
  ...
  for (const id of source) {
    if (targetSet.has(id)) {
      if (index >= replacement.length)
        throw new Error("replacement order is shorter than target order");
  ...
  if (index !== replacement.length)
    throw new Error("replacement order is longer than target order");
