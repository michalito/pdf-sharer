...
describe("getSettingsSnapshot", () => {
  ...
  test("merges partial settings with defaults", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultSortField: "created" }));
    const snapshot = getSettingsSnapshot();
  ...
  test("reads stored state filter value", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultStateFilter: "done" }));
    expect(getSettingsSnapshot().defaultStateFilter).toBe("done");
  });

  test("reads 'all' state filter value", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultStateFilter: "all" }));
    expect(getSettingsSnapshot().defaultStateFilter).toBe("all");
  });

  test("ignores invalid state filter values", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultStateFilter: "bogus" }));
    expect(getSettingsSnapshot().defaultStateFilter).toBe("active");
  });

  test("reads stored perPage value", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultPerPage: 100 }));
    expect(getSettingsSnapshot().defaultPerPage).toBe(100);
  });

  test("ignores invalid perPage values", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultPerPage: 42 }));
  ...
  test("reads stored empty-string TTL as 'never'", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultTtl: "" }));
    expect(getSettingsSnapshot().defaultTtl).toBe("");
  });

  test("ignores invalid TTL values", () => {
    localStorage.setItem("saita-settings", JSON.stringify({ defaultTtl: "2h" }));
    expect(getSettingsSnapshot().defaultTtl).toBe("");
  });
});
...
describe("useSettings", () => {
  ...
  test("persists updated sort field to localStorage", () => {
    ...
    expect(JSON.parse(localStorage.getItem("saita-settings")!).defaultSortField).toBe("name");
  });
  ...
  test("persists updated sort order to localStorage", () => {
    ...
    expect(JSON.parse(localStorage.getItem("saita-settings")!).defaultSortOrder).toBe("asc");
  });
  ...
  test("persists updated state filter to localStorage", () => {
    ...
    expect(JSON.parse(localStorage.getItem("saita-settings")!).defaultStateFilter).toBe("archived");
  });
  ...
  test("persists updated perPage to localStorage", () => {
    ...
    expect(JSON.parse(localStorage.getItem("saita-settings")!).defaultPerPage).toBe(200);
  });
  ...
  test("persists updated TTL to localStorage", () => {
    ...
    expect(JSON.parse(localStorage.getItem("saita-settings")!).defaultTtl).toBe("24h");
  });
